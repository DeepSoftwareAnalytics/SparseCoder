...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        source_tokens = self.tokenizer.tokenize(source,truncation=True)[:self.args.max_source_length-2]
        ...
        target_tokens = self.tokenizer.tokenize(target,truncation=True)[:self.args.max_target_length-2]
...
def main():
    ...
    encoder, tokenizer = converter.convert_from_pretrained(args.model_name_or_path, num_global_tokens=7)
    ...
    model=Seq2Seq(encoder=encoder,decoder=decoder,embeddings=model.embeddings,config=config,tokenizer=tokenizer,
