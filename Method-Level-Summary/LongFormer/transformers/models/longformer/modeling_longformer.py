...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        self.query_global_lora_in = nn.Linear(config.hidden_size, 8,bias=False)
        self.query_global_lora_out = nn.Linear(8, self.embed_dim,bias=False)
        
        self.key_global_lora_in = nn.Linear(config.hidden_size, 8,bias=False)
        self.key_global_lora_out = nn.Linear(8, self.embed_dim,bias=False)

        self.value_global_lora_in = nn.Linear(config.hidden_size, 8,bias=False)
        self.value_global_lora_out = nn.Linear(8, self.embed_dim,bias=False)
    ...
    def forward(
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        if identifier_mask.sum().item()!=0:
            float_mask = identifier_mask[:,:,None,None].type_as(query_vectors).masked_fill(
                identifier_mask[:,:,None,None], torch.finfo(query_vectors.dtype).min
            )
            # diagonal mask with zeros everywhere and -inf inplace of padding
            diagonal_mask2 = self._sliding_chunks_query_key_matmul(
                identifier_mask[:,:,None,None], float_mask, self.one_sided_attn_window_size
            )
            # pad local attention probs
            attn_scores += diagonal_mask + diagonal_mask2
        else:
        ...
        if is_global_attn:
            ...
            if identifier_mask.sum().item()!=0:
                (
                    max_num_global_attn_indices_identifier,
                    is_index_global_attn_nonzero_identifier,
                    is_local_index_global_attn_nonzero_identifier,
                    is_local_index_no_global_attn_nonzero_identifier,
                ) = self._get_global_attn_indices(identifier_mask)

                query_vectors_identifier = hidden_states.new_zeros(
                    batch_size, max_num_global_attn_indices_identifier, self.num_heads,self.head_dim
                )
                query_vectors_identifier[is_local_index_global_attn_nonzero_identifier] = query_vectors[is_index_global_attn_nonzero_identifier]

                identifier_key_attn_scores = self._concat_with_global_key_attn_probs(
                    query_vectors=query_vectors_identifier,
                    key_vectors=key_vectors,
                    max_num_global_attn_indices=max_num_global_attn_indices_identifier,
                    is_index_global_attn_nonzero=is_index_global_attn_nonzero_identifier,
                    is_local_index_global_attn_nonzero=is_local_index_global_attn_nonzero_identifier,
                    is_local_index_no_global_attn_nonzero=is_local_index_no_global_attn_nonzero_identifier,
                )


                tmp = attn_scores.new_zeros(batch_size,max_num_global_attn_indices_identifier,self.num_heads,attn_scores.size(-1)).fill_(torch.finfo(query_vectors.dtype).min)
                tmp[is_local_index_global_attn_nonzero_identifier] = attn_scores[is_index_global_attn_nonzero_identifier]
                
                identifier_attn_scores = torch.cat((identifier_key_attn_scores, tmp), dim=-1)
                del identifier_key_attn_scores
                del tmp
            # free memory
            del global_key_attn_scores

        if identifier_mask.sum().item()!=0:
            tmp = nn.functional.softmax(
                attn_scores, dim=-1, dtype=torch.float32
            )  # use fp32 for numerical stability
            identifier_attn_probs = nn.functional.softmax(
                identifier_attn_scores, dim=-1, dtype=torch.float32
            )  # use fp32 for numerical stability

            (
                max_num_global_attn_indices_identifier1,
                is_index_global_attn_nonzero_identifier1,
                is_local_index_global_attn_nonzero_identifier1,
                is_local_index_no_global_attn_nonzero_identifier1,
            ) = self._get_global_attn_indices(~identifier_mask)

            attn_probs = tmp.new_zeros(batch_size, seq_len, self.num_heads,tmp.size(-1))
            attn_probs[is_index_global_attn_nonzero_identifier1] = tmp[is_index_global_attn_nonzero_identifier1]
            attn_probs[is_index_global_attn_nonzero_identifier] = identifier_attn_probs[:,:,:,-attn_probs.size(-1):][is_local_index_global_attn_nonzero_identifier]
            identifier_attn_probs = identifier_attn_probs[:,:,:,:-attn_probs.size(-1)]
            del tmp
        else:
        ...
        if layer_head_mask is not None:
            ...
            if identifier_mask.sum().item()!=0:
                identifier_attn_probs = layer_head_mask.view(1, 1, -1, 1) * identifier_attn_probs

        # softmax sometimes inserts NaN if all positions are masked, replace them with 0
        attn_probs = torch.masked_fill(attn_probs, is_index_masked[:, :, None, None], 0.0)
        attn_probs = attn_probs.type_as(attn_scores)
        if identifier_mask.sum().item()!=0:
            identifier_attn_probs = identifier_attn_probs.type_as(identifier_attn_scores)

        # free memory
        del attn_scores
        if identifier_mask.sum().item()!=0:
            del identifier_attn_scores

        # apply dropout
        attn_probs = nn.functional.dropout(attn_probs, p=self.dropout, training=self.training)
        if identifier_mask.sum().item()!=0:
            identifier_attn_probs = nn.functional.dropout(identifier_attn_probs, p=self.dropout, training=self.training)

        value_vectors = value_vectors.view(seq_len, batch_size, self.num_heads, self.head_dim).transpose(0, 1)
        if identifier_mask.sum().item()!=0:
            value_vectors_identifier = hidden_states.new_zeros(
                batch_size, max_num_global_attn_indices_identifier, self.num_heads,self.head_dim
            )
            value_vectors_identifier[is_local_index_global_attn_nonzero_identifier] = value_vectors[is_index_global_attn_nonzero_identifier]
        ...
        if is_global_attn:
            ...
            if identifier_mask.sum().item()!=0:
                identifier_attn_output = torch.einsum("abcd,adce->abce",identifier_attn_probs,value_vectors_identifier)
                attn_output[is_index_global_attn_nonzero_identifier] += identifier_attn_output[is_local_index_global_attn_nonzero_identifier]
        else:
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        global_query_vectors_only_global = self.query(global_attn_hidden_states) + self.query_global_lora_out(self.query_global_lora_in(global_attn_hidden_states))
        global_key_vectors = self.key(hidden_states) + self.key_global_lora_out(self.key_global_lora_in(hidden_states))
        global_value_vectors = self.value(hidden_states) + self.value_global_lora_out(self.value_global_lora_in(hidden_states))
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    def forward(
        ...
        identifier_mask: Optional[torch.Tensor] = None,
